...
exports.validarDisponibilidadSlots = exports.onTurnoEstadoCambio = void 0;
 ...
 */
exports.onTurnoEstadoCambio = functions
    ...
    if (!after)
        return; // Turno eliminado, ignorar
    const estadoAnterior = before === null || before === void 0 ? void 0 : before.estado;
    ...
    if (estadoNuevo === 'COMPLETADA' && estadoAnterior !== 'COMPLETADA') {
        ...
        // Notificar al alumno
        await crearNotificacion(alumnoUid, 'clase_completada', 'Clase completada', 'Tu clase fue completada. Podés calificar al instructor ahora.');
    ...
    if (estadoNuevo === 'CONFIRMADA' && estadoAnterior === 'PENDIENTE_CONFIRMACION') {
        await crearNotificacion(alumnoUid, 'confirmacion_turno', 'Clase confirmada', `Tu clase del ${after.fechaStr} a las ${after.horaInicio} fue confirmada.`);
    }
    // ── RECHAZADA: notificar al alumno ─────────────────────────────────────────
    if (estadoNuevo === 'RECHAZADA' && estadoAnterior === 'PENDIENTE_CONFIRMACION') {
        const motivo = (_a = after.motivoRechazo) !== null && _a !== void 0 ? _a : 'Sin motivo especificado';
        await crearNotificacion(alumnoUid, 'rechazo_turno', 'Clase rechazada', `Tu solicitud del ${after.fechaStr} fue rechazada. Motivo: ${motivo}`);
    }
    // ── PENDIENTE_CONFIRMACION: notificar al instructor ────────────────────────
    if (estadoNuevo === 'PENDIENTE_CONFIRMACION' && !estadoAnterior) {
        await crearNotificacion(instructorUid, 'nueva_solicitud', 'Nueva solicitud de clase', `Tenés una nueva solicitud para el ${after.fechaStr} a las ${after.horaInicio}.`);
    }
});
/**
 ...
 */
exports.validarDisponibilidadSlots = functions
    .region('southamerica-east1')
    .https.onCall(async (data, context) => {
    if (!context.auth)
        throw new functions.https.HttpsError('unauthenticated', 'Debe estar autenticado.');
    const { instructorUid, fechaStr, slots } = data;
    if (!instructorUid || !fechaStr || !(slots === null || slots === void 0 ? void 0 : slots.length)) {
    ...
    snap.docs.forEach(d => {
        var _a;
        const t = d.data();
        (_a = t.slots) === null || _a === void 0 ? void 0 : _a.forEach(s => ocupados.add(s));
    });
    const conflicto = slots.some(s => ocupados.has(s));
    return { disponible: !conflicto };
});
// ─── Helpers ──────────────────────────────────────────────────────────────────
async function consumirClase(alumnoUid, consumidoDe) {
    ...
    await db.runTransaction(async (tx) => {
        ...
        if (consumidoDe === 'plan' && alumnoData.planContratado) {
            ...
            if (restantes < 0)
                throw new Error('Sin clases disponibles en el plan');
            tx.update(userRef, {
            ...
            if (disponibles < 0)
                throw new Error('Sin crédito individual disponible');
            tx.update(userRef, {
...
async function crearNotificacion(userId, tipo, titulo, mensaje, turnoId) {
    await db.collection('notificaciones').add({
        ...
        mensaje,
        leida: false,
        turnoId: turnoId !== null && turnoId !== void 0 ? turnoId : null,
        creadoEn: admin.firestore.FieldValue.serverTimestamp(),
    });
}
